...
echo "item = $item "
#if [ "$item" = "slprs" ]    | [ "$item" = "brtmp"  ] | [ "$item" = "ice" ]; then
if [ "$item" = "slprs" ]; then
   orgdir=../linkdir/work/jra55fcst_filt_3hr_TL319       # slprs brtmp ice
elif [ "$item" = "brtmp" ]; then
   orgdir=../linkdir/work/jra55fcst_filt_3hr_TL319       # slprs brtmp ice
elif [ "$item" = "ice" ]; then
   orgdir=../linkdir/work/jra55fcst_filt_3hr_TL319       # slprs brtmp ice
elif [ "$item" = "tmp10m" ]; then
   orgdir=../linkdir/work/jra55fcst_v1_3_prod3_3hr_TL319 # tmp10m sph10m
elif [ "$item" = "sph10m" ] ; then
   orgdir=../linkdir/work/jra55fcst_v1_3_prod3_3hr_TL319 # tmp10m sph10m
elif [ "$item" = "u10m"  ] ; then
   orgdir=../linkdir/work/jra55fcst_v1_3_prod1_3hr_TL319 # u10m v10m
elif [ "$item" = "v10m"  ] ; then
   orgdir=../linkdir/work/jra55fcst_v1_3_prod1_3hr_TL319 # u10m v10m
elif [ "$item" = "dswrf" ] ; then
   orgdir=../linkdir/work/jra55fcst_v1_3_rad2_3hr_TL319  # dswrf dlwrf v1_3 => const correction
elif [ "$item" = "dlwrf" ] ; then
   orgdir=../linkdir/work/jra55fcst_v1_3_rad2_3hr_TL319  # dswrf dlwrf v1_3 => const correction
elif [ "$item" = "prcp" ]  ; then
   orgdir=../linkdir/work/jra55fcst_v1_3_prcp2_3hr_TL319 # rain snow ("rain" output uses "prcp" input)
elif [ "$item" = "snow" ]  ; then
   orgdir=../linkdir/work/jra55fcst_v1_3_prcp2_3hr_TL319 # rain snow 
else
  echo "ERROR: unrecognized item = $item "
fi
